...
const Pedido = ({ pedido }) => {
    ...
    // Mutation para eliminar un pedido, para jugar con el cache necesitas la consulta padre
    const [eliminarPedido] = useMutation(ELIMINAR_PEDIDO, {
        update(cache) {
            // Primero lees el cache, despues lo escribes
            ...
            cache.writeQuery({
                query: OBTENER_PEDIDOS,
                // data: Que es lo que vamos a reescribir
                data: {
                    obtenerPedidosVendedor: obtenerPedidosVendedor.filter(pedidoRecorido => pedidoRecorido.id !== id)
                }
            })
        }
    })
    ...
    return (
        <div className={`${clase} border-t-4 mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg`}>
            ...
            <div>
                ...
                <button className='flex intems-center mt-4 bg-red-800 px-5 py-2 inline-block text-white rounded leading-tight uppercase text-xs font-bold' onClick={confirmarEliminarPedido}>
                    Eliminar pedido
