...
const NuevoCliente = () => {
    ...
    const [nuevoCliente] = useMutation(NUEVO_CLIENTE, {
        // El cache se ira actualizando con forme se agregen nuevos clientes
        update(cache, { data: { nuevoCliente } }) {
            // Obtener el objeto de cache que deseeas actualizar
            const { obtenerClientesVendedor } = cache.readQuery({ query: OBTENER_CLIENTES_USUARIO })

            // Reescribimos el cache ( El cache nunca se debe modificar, se reescribe )
            cache.writeQuery({
    ...
    const formik = useFormik({
        ...
        onSubmit: async valores => {
            ...
            try {
                // Data es la respuesta del mutation
